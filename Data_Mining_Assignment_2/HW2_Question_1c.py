...
testRowID = test_df[test_df.columns[0]].copy()
...
for kValue in range(len(k_values)) :
    indexCounter = 0
    predictedLabel = []
    for row in test_dfNormalized.itertuples(index=False, name='Pandas'):
        ...
        nnIndex = distance_dfForRow.loc[:(k_values [kValue] - 1) ,'train_row_index']
    ...
    tmpList = { k_values [kValue] : predictedLabel}
    predictedTestLabel = pd.DataFrame(tmpList, index=testRowID[:50].values)

    resultDF[str(k_values [kValue])] = predictedTestLabel
